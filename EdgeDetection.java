...
public class EdgeDetection {
	...
	private HashMap<String, double[][]> buildFilterMap() {
		HashMap<String, double[][]> temp_hashmap = new HashMap<String, double[][]>();
		temp_hashmap.put(VERTICAL_FILTER, FILTER_VERTICAL);
		temp_hashmap.put(HORIZONTAL_FILTER, FILTER_HORIZONTAL);
		temp_hashmap.put(SOBEL_FILTER_VERTICAL, FILTER_SOBEL_V);
		temp_hashmap.put(SOBEL_FILTER_HORIZONTAL, FILTER_SOBEL_H);
		temp_hashmap.put(SCHARR_FILTER_VERTICAL, FILTER_SCHARR_V);
		temp_hashmap.put(SCHARR_FILTER_HORIZONTAL, FILTER_SCHARR_H);
		return temp_hashmap;
	}
	
	private BufferedImage applyMedainFilter(BufferedImage bufferedImage) {
	...
	public BufferedImage detectEdges(BufferedImage bufferedImage) throws IOException {
		bufferedImage = applyMedainFilter(bufferedImage);
		double[][][] image = transformImageToArray(bufferedImage);
		double[][] filter = filterMap.get(SOBEL_FILTER_HORIZONTAL);
